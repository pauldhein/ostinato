...
from src.ostinato.music_utils import (
    get_pitch_val,
    get_key_type,
    get_key_root,
    get_bpm,
)
from src.ostinato.music_utils import assign_dur_label  # determine_duration
from src.ostinato.music_utils import KeyType, MusicElement
...
class XMLParser(object):
    ...
    def check_filepath(self, fp):
        ...
        if fp.endswith(".mxl"):
            ...
            files = list(
                filter(
                    lambda x: x.endswith(".xml") and "/" not in x,
                    zipper.namelist(),
                )
            )
    ...
    def parse_music_scores(self, music_path="./", save_name="parker_data"):
        ...
        files = [
            join(root, elm)
            for root, dirs, files in walk(music_path)
            for elm in files
        ]
        xml_files = filter(lambda x: x.endswith((".xml", ".mxl")), files)
        scores = {
            fp[fp.rfind("/") + 1 :]: self.parse_score(fp) for fp in xml_files
        }
        return scores
    ...
    def parse_measure(self, measure, m_num):
        ...
        def parse_music_element(music_el):
            ...
            for item in music_el:
                if item.tag == "chord":
                    ...
                    octave = (
                        int(item[2].text) if has_alt else int(item[1].text)
                    )
                    midi = get_pitch_val(item[0].text, octave, alteration)
                    if not isinstance(midi, int):
                        print(
                            "Midi pitch value returned {0} of type {1}".format(
                                midi, type(midi)
                            )
                        )
                elif item.tag == "rest":
        ...
        for music_el in measure:
            if music_el.tag == "note":
                ...
                if (
                    element[0] == MusicElement.CHORD
                ):  # guards the increment for sum_dur
                    last = result["melody"].pop()
                    pitches = (
                        (last[1],) + (element[1],)
                        if not isinstance(last[1], tuple)
                        else last[1] + (element[1],)
                    )
        ...
        harmony = [
            (r, k, (m, o / sum_dur)) for (r, k, (m, o)) in result["harmony"]
        ]
    ...
    def parse_score(self, filepath=""):
        ...
        def parse_direction(part):
            ...
            for music_el in part[0]:
                if (
                    music_el.tag == "direction"
                    and music_el[0].tag == "direction-type"
                    and music_el[0][0].tag == "metronome"
                ):
